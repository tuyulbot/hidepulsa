...
#@bot.on(events.NewMessage(pattern=r"(?:.bycategory_isat|/bycategory_isat)$"))
...
async def bycategory_isat(event):
    ...
    async with bot.conversation(chat) as conv:
        ...
        for i, p in enumerate(produk_list, start=1):
            ...
            payment_support = p["payment_suport"]
        ...
        try:
            ...
            for task in done:
                ...
                if isinstance(result, events.CallbackQuery.Event):
                    ...
                    _, idx = result.data.decode().split("|", 1)
                    idx = int(idx) - 1
        ...
        try:
            while True: 
                ...
                for task in done:
                    cq = task.result()
                    await cq.answer()
                    if isinstance(cq, events.CallbackQuery.Event):
                        if cq.data == b"menu":
                            await clear_session(user_id)
                            asyncio.create_task(auto_delete_multi(user_id, 0, fetching_msg, nomor_event.message, prompt))
                            return

                        if cq.data == b"kembali_list":
                            await cq.edit(
                                "\n".join(text_lines),
                                buttons=buttons
                            )
                        ...
                        action, idx = cq.data.decode().split("|", 1)
                        idx = int(idx) - 1
                        ...
                        if action == "detail_list":
                            ...
                            text = [
                                f"📦 **{data['package_name']}**",
                                f"💰 {data['tariff_display']}",
                                f"⏳ {data['duration']}",
                                "",
                                "📝 **Deskripsi Paket:**"
                            ]

                            for d in data["deskripsi"]:
                                text.append(f"• {d}")
        ...
        try:
            ...
            for task in done:
                ...
                if isinstance(result, events.CallbackQuery.Event):
                    ...
                    _, payment_channel = result.data.decode().split("|", 1)
        ...
        """pay_msg = await event.respond(
            f"✅ **Paket dipilih**\n\n"
            f"📦 {nama_paket}\n"
            f"🧾 Kode Buy: `{kode_buy}`\n\n"
            f"💳 payment: `{payment_channel}`"
            )"""

        proses_msg = await event.respond(
            "⏳ Sedang memproses pembelian tunggu sampe proses selesai"  
        )
        ...
        if status != "success" or code != 0:
            msg = (
                data_message
                or result.get("message")
                or stderr
                or "Terjadi kesalahan saat memproses transaksi."
            )
        ...
        # ===== E-WALLET =====
        if payment_channel in ["dana", "ovo", "gopay", "shopee"]:
        ...
        # ===== PULSA =====
        if payment_channel == "pulsa":
            msg = init.get("message", "Transaksi sedang diproses")
            transid = init.get("transid", "-")
