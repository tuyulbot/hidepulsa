...
# ==== Helper: reset keranjang & purge sesi checkout user ====
def _hard_reset_cart(user_id: int):
    # kosongkan keranjang user
    user_carts[user_id] = {"items": {}, "created": time.time(), "updated": time.time()}

    # hapus semua sesi checkout lama milik user ini (kalau ada)
    # pola key: "checkout:<user_id>:xxxx"
...
def format_error_response(method, nomor_mask, art):
...
def _cart_supported_payments(cart: dict) -> list[str]:
    pays = set()
...
def _fill_from_inner(out, inner):
...
def extract_payment_artifacts(raw_res: dict) -> dict:
    out = {
        "payment_method": "-",
        "qr_code": None,
        "deeplink": None,
    ...
    st = (raw_res.get("status")
          or _dig(raw_res, ["data", "status"])
          or _dig(raw_res, ["result", "status"])
          or "-")
    out["status"] = str(st).lower()

    # ---- (A) model lama: res.data.data[.data]
    level1 = _dig(raw_res, ["data", "data"], {})
    if level1:
        inner = _leaf_data(level1)              # <<— masuk ke .data kalau ada
        _fill_from_inner(out, inner)
        qro = _dig(raw_res, ["data", "qr_code"])
    ...
    def _pull_from_output(output_block: dict):
        ...
        if isinstance(j, dict):
            j_status = (j.get("status") or _dig(j, ["data", "status"]))
            ...
            if isinstance(q, str) and q.strip():
                out["qr_code"] = q.strip()
            # bawa data.data[.data]
            inner_j = _leaf_data(_dig(j, ["data", "data"], {}))
            _fill_from_inner(out, inner_j)

        # b) stdout string JSON
        s = output_block.get("stdout")
        if isinstance(s, str) and s.strip().startswith("{"):
            sj = _safe_json_loads(s)
            if isinstance(sj, dict):
                sj_status = (sj.get("status") or _dig(sj, ["data", "status"]))
                ...
                if isinstance(q, str) and q.strip():
                    out["qr_code"] = q.strip()
                inner_sj = _leaf_data(_dig(sj, ["data", "data"], {}))
                _fill_from_inner(out, inner_sj)

    # ---- (B) job wrapper di root: res.output.*
    _pull_from_output(_dig(raw_res, ["output"], {}))

    # ---- (C) job wrapper di dalam result: res.result.output.*
    _pull_from_output(_dig(raw_res, ["result", "output"], {}))

    # ---- (D) accepted → processing (fallback)
    if out["status"] in ("-", "") and (_dig(raw_res, ["data", "status"]) or "").lower() == "accepted":
...
async def poll_process_status(process_id: str, api_key: str, *, interval: int = 30, timeout: int = 1800) -> dict:
    """
...
async def detail_produk(event):
    ...
    btns = [
        ...
        Button.inline("⬅️ Kembali", f"page|{keyword}|{page}".encode())],
...
async def build_produk_page(user_id, keyword, produk_list, page=1):
    ...
    nav_buttons = []
    if page > 1:
        nav_buttons.append(Button.inline("⏮️ Prev", f"page|{keyword}|{page-1}".encode()))
    if page < total_pages:
        nav_buttons.append(Button.inline("⏭️ Next", f"page|{keyword}|{page+1}".encode()))
    if nav_buttons:
...
async def refresh_catalog_keyboard(user_id: int):
    ...
    try:
        ...
        _, buttons = await build_produk_page(user_id, state["keyword"], produk_list, page=state["page"])
...
# =========================================================
@bot.on(events.CallbackQuery(pattern=b'methodbuylegal\|(.+)'))
async def methodbuylegal(event):
    await event.delete()
    user_id = event.sender_id
    ...
    description, buttons = await build_produk_page(user_id, keyword, produk_list, page=1)
...
async def change_page(event):
    ...
    description, buttons = await build_produk_page(user_id, keyword, produk_list, page)
    new_message = await event.respond(description, buttons=buttons)
...
async def add_to_cart(event):
    ...
    await refresh_catalog_keyboard(user_id)  # badge N real-time
...
async def back_to_catalog(event):
    ...
    description, buttons = await build_produk_page(user_id, state["keyword"], produk_list, page=state["page"])
    new_message = await event.respond(description, buttons=buttons)
...
async def checkout(event):
    ...
    # Simpan sesi checkout (keranjang + nomor)
    await _delete_last_message(user_id)

    checkout_key = f"checkout:{user_id}:{uuid.uuid4().hex[:6]}"
    user_sessions[checkout_key] = {"cart": cart, "numbers": daftar_nomor, "created": time.time()}
    asyncio.create_task(expire_session(checkout_key))

    pays = _cart_supported_payments(cart) or ["pulsa"]  # union; ganti ke intersection kalau mau

    buttons, row = [], []
    for i, pay in enumerate(pays, 1):
        emoji = EMOJI_PAYMENT.get(pay, "💳")
        row.append(Button.inline(f"{emoji} {pay.title()}", f"paycart|{checkout_key}|{pay}".encode()))
        if i % 2 == 0:
...
async def proses_beli_cart(event):
    user_id = event.sender_id
    chat = event.chat_id
    _, checkout_key, payment = event.data.decode().split("|", 2)

    info = user_sessions.get(checkout_key)
    if not info:
        err = await event.respond("❌ Session checkout tidak ditemukan / expired.")
        user_messages[user_id] = err
    ...
    try:
        del user_sessions[checkout_key]
    except KeyError:
        pass
    ...
    for p_idx, item in enumerate(items, start=1):
        ...
        hdr = await event.respond(f"🧺 Paket {p_idx}/{len(items)}: **{nama_paket}** (panel {rupiah(harga_panel)})")
        ...
        for n_idx, nomor_hp in enumerate(numbers, start=1):
            ...
            if saldo < harga_panel:
                ...
                warn = await event.respond(
                    f"❌ [{p_idx}.{n_idx}] `{nomor_mask}` saldo kurang. Harga: {rupiah(harga_panel)} | Saldo: {rupiah(saldo)} | Kurang: {rupiah(kurang)}",
                    parse_mode="markdown"
                )
                asyncio.create_task(auto_delete_multi(user_id, 25, warn))
                if n_idx < len(numbers): await asyncio.sleep(BATCH_DELAY_PER_NOMOR)
            ...
            status_awal = (_dig(res, ["data","status"]) or res.get("status") or "").lower()
            process_id  = _dig(res, ["data","processId"]) or res.get("processId")
            ...
            if status_awal == "accepted" and process_id:
                ...
                final = await poll_process_status(process_id, user_data["api_key"], interval=30, timeout=30*60)
                ...
                art = extract_payment_artifacts(final)
            else:
                # bukan processId, langsung normalisasi res awal
                art = extract_payment_artifacts(res)
            ...
            laporan = (
                ...
                f"├ 💵 Harga Pkt  : {rupiah(harga_total or harga_panel)}\n"
            ...
            if status_final == "SUCCESS":
                ...
                if pmethod == "QRIS" or (payment == "qris"):
                    ...
                    if qr_code_str:
                        ...
                        warn = await event.respond(format_error_response(pmethod or payment, nomor_hp, art), parse_mode="markdown")
