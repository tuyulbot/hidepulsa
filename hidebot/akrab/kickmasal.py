...
# ====== UTIL PARSER NOMOR (dipakai di semua jalur) ======
def parse_numbers(text: str, normalize: bool = True, limit: int = MAX_NUMBERS) -> list[str]:
    if not text:
        return []
    # split spasi/koma/newline
...
@bot.on(events.NewMessage(pattern=r"(?:\.kickmasal|/kickmasal)(?:\s+(.+))?$"))
...
async def kickmasal_slot(event):
    ...
    if saldo < 10000:
        ...
        sid_tmp = f"kickmasal:{secrets.token_hex(2)}"
        user_sessions.setdefault(user_id, {})[sid_tmp] = {
            "messages": [msg],
            "created_at": time.time()
        }
        asyncio.create_task(expire_session(user_id, sid_tmp, 20))
        return

    # --------- Ambil kandidat nomor dari: argumen / reply / (fallback: nanti conversation) ----------
    arg_text = None
    if isinstance(event, events.NewMessage.Event):
        # grup pertama dari regex -> argumen setelah command
        try:
            arg_text = (event.pattern_match.group(1) or "").strip()
        except Exception:
            arg_text = ""
    elif isinstance(event, events.CallbackQuery.Event):
        arg_text = ""  # tombol tidak bawa teks nomor

    reply_text = ""
    try:
        if await event.get_reply_message():
            reply_text = (await event.get_reply_message()).message or ""
    except Exception:
        reply_text = ""

    initial_numbers = parse_numbers(arg_text, NORMALIZE_08_TO_62) or parse_numbers(reply_text, NORMALIZE_08_TO_62)
    ...
    # ====== UTIL TAMPIL INFO SLOT (panel garis) ======
    async def show_info_slot(nomor_admin: str, expired_val: str, slot_list: list[dict]):
        ...
        # sengaja tidak dikirim satu-satu agar tidak spam; panggil ini kalau mau menampilkan
    ...
    # ====== Jalur 1: nomor sudah ada (argumen / reply) -> langsung proses tanpa prompt ======
    if initial_numbers:
        found_msg = await event.respond(f"🔄 Ditemukan {len(initial_numbers)} nomor. Menyiapkan proses…")
        user_sessions[user_id][sid]["messages"].append(found_msg)

        progress_msgs = []
        for i, nadmin in enumerate(initial_numbers, start=1):
            progress = await event.respond(
                f"🧩 Proses [{i}/{len(initial_numbers)}] untuk **{nadmin}**…",
                parse_mode="markdown"
            )
        ...
        # auto delete yang sementara
        asyncio.create_task(auto_delete_multi(user_id, 5, found_msg, *progress_msgs))
        asyncio.create_task(expire_session(user_id, sid, 5))
        return

    # ====== Jalur 2: tidak ada nomor -> fallback ke mode percakapan ======
