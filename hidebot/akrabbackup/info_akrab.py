...
async def akrabbekasan_slot(event):
    ...
    if saldo < 10000:
        ...
        user_sessions.setdefault(user_id, {}).setdefault("akrabbekasan", {"messages": []})
        user_sessions[user_id]["akrabbekasan"]["messages"].append(msg)
        asyncio.create_task(expire_session(user_id, "akrabbekasan", 20))
        return

    # Simpan session
    user_sessions.setdefault(user_id, {})["akrabbekasan"] = {"messages": [], "created_at": time.time()}
    ...
    user_sessions[user_id]["akrabbekasan"]["messages"].append(prompt)
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            if not nomor_list:
                ...
                user_sessions[user_id]["akrabbekasan"]["messages"].append(gagal_msg)
                asyncio.create_task(expire_session(user_id, "akrabbekasan", 30))
                return
            ...
            user_sessions[user_id]["akrabbekasan"]["messages"].append(error)
            asyncio.create_task(expire_session(user_id, "akrabbekasan", 30))
            return
    ...
    user_sessions[user_id]["akrabbekasan"]["messages"].append(fetching_msg)

    asyncio.create_task(auto_delete_multi(user_id, 5, fetching_msg, nomor_event.message, prompt)) 
    asyncio.create_task(expire_session(user_id, "akrabbekasan", 5))
