...
@bot.on(events.NewMessage(pattern=r"(?:.addedit|/addedit)$"))
@bot.on(events.CallbackQuery(pattern=b'addedit'))
async def addedit_slot(event):
    ...
    if saldo < 10000:
        ...
        user_sessions.setdefault(user_id, {}).setdefault("addedit", {"messages": []})
        user_sessions[user_id]["addedit"]["messages"].append(msg)
        asyncio.create_task(expire_session(user_id, "addedit", 20))
        return

    # Simpan session awal
    user_sessions.setdefault(user_id, {})["addedit"] = {"messages": [], "created_at": time.time()}

    prompt = await event.respond("📲 Silakan kirim nomor pengelola :", buttons=[
            [Button.inline("❌ Cancel", b"menu")]
        ]
    )
    user_sessions[user_id]["addedit"]["messages"].append(prompt)
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            user_sessions[user_id]["addedit"]["messages"].append(error)
            asyncio.create_task(expire_session(user_id, "addedit", 30))
            return
        ...
        if not nomor_hp.isdigit():
            ...
            user_sessions[user_id]["addedit"]["messages"].append(gagal_msg)
            asyncio.create_task(expire_session(user_id, "addedit", 30))
            return

        fetching_msg = await event.respond("🔄 Proses mengambil data slot …")
        user_messages[user_id] = fetching_msg
        user_sessions[user_id]["addedit"]["messages"].append(fetching_msg)
        ...
        user_sessions[user_id]["addedit"]["messages"].append(choose)
        ...
        user_sessions[user_id]["addedit"]["messages"].append(prompt2)
        ...
        user_sessions[user_id]["addedit"]["messages"].append(prompt3)
        ...
        user_sessions[user_id]["addedit"]["messages"].append(prompt4)
        ...
        prompt5 = await event.respond("📲 Silakan kirim GB kuota bersama (20 = 20GB) :")
        user_sessions[user_id]["addedit"]["messages"].append(prompt2)
        user_messages[user_id] = prompt2
        try:
            done, pending = await asyncio.wait(
            ...
            user_messages[user_id] = nomor_event5.message
            total_kuber = nomor_event5.text.strip()
        ...
        proses_msg = await event.respond(
            ...
            f"├ 💳 Slot : {no_slot}\n"
            f"├ 🗃 Kuber : {total_kuber} GB\n"
            f"└ 💰 Saldo : Rp {saldo:,}\n\n"
            "⏳ Sedang memproses tunggu sampe proses selesai"  
        )
        user_messages[user_id] = proses_msg
        user_sessions[user_id]["addedit"]["messages"].append(proses_msg)

        asyncio.create_task(auto_delete_multi(user_id, 5, choose, fetching_msg, nomor_event.message, prompt, prompt2, nomor_event1.message, prompt3, nama_adminn.message, prompt4, nama_anggotaa.message, proses_msg, prompt5, nomor_event5.message))
        asyncio.create_task(expire_session(user_id, "addedit", 5))
        ...
        final_add = (
            ...
            f"└ 📝 Note : {note_exec}\n"
            "╰━━━━━━━━━━━━━━━━━━━━╯\n\n"
            " ⌛ Sedang proses edit kuber, mohon tunggu 15 detik..."
        )
        msg = await event.respond(final_add, parse_mode="markdown")
        user_messages[user_id] = msg

        await asyncio.sleep(15)  # tunggu 15 detik sebelum melanjutkan
        ...
        await msg.edit(final_edit, parse_mode="markdown")
