...
@bot.on(events.NewMessage(pattern=r"(?:.addkick|/addkick)$"))
@bot.on(events.CallbackQuery(pattern=b'addkick'))
async def addkick_slot(event):
    ...
    if saldo < 10000:
        ...
        user_sessions.setdefault(user_id, {}).setdefault("addkick", {"messages": []})
        user_sessions[user_id]["addkick"]["messages"].append(msg)
        asyncio.create_task(expire_session(user_id, "addkick", 20))
        return

    # Simpan session awal
    user_sessions.setdefault(user_id, {})["addkick"] = {"messages": [], "created_at": time.time()}

    prompt = await event.respond("📲 Silakan kirim nomor pengelola :", buttons=[
            [Button.inline("❌ Cancel", b"menu")]
        ]
    )
    user_sessions[user_id]["addkick"]["messages"].append(prompt)
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            user_sessions[user_id]["addkick"]["messages"].append(error)
            asyncio.create_task(expire_session(user_id, "addkick", 30))
            return
        ...
        if not nomor_hp.isdigit():
            ...
            user_sessions[user_id]["addkick"]["messages"].append(gagal_msg)
            asyncio.create_task(expire_session(user_id, "addkick", 30))
            return

        fetching_msg = await event.respond("🔄 Proses mengambil data slot …")
        user_messages[user_id] = fetching_msg
        user_sessions[user_id]["addkick"]["messages"].append(fetching_msg)
        ...
        user_sessions[user_id]["addkick"]["messages"].append(choose)
        ...
        user_sessions[user_id]["addkick"]["messages"].append(prompt2)
        ...
        user_sessions[user_id]["addkick"]["messages"].append(prompt3)
        ...
        user_sessions[user_id]["addkick"]["messages"].append(prompt4)
        ...
        user_sessions[user_id]["addkick"]["messages"].append(proses_msg)
        ...
        asyncio.create_task(expire_session(user_id, "addkick", 5))
        ...
        final_add = (
            ...
            f"└ 📝 Note : {note_exec}\n"
            "╰━━━━━━━━━━━━━━━━━━━━╯\n]n"
            " ⌛ Sedang proses add + kick, mohon tunggu 15 detik...\n\n"
        )
        ...
        slot_exp = details.get("slot_expiration")
