...
@bot.on(events.CallbackQuery(pattern=b'bekasankick'))
async def bekasankick_slot(event):
    ...
    if saldo < 10000:
        ...
        user_sessions.setdefault(user_id, {}).setdefault("bekasankick", {"messages": []})
        user_sessions[user_id]["bekasankick"]["messages"].append(msg)
        asyncio.create_task(expire_session(user_id, "bekasankick", 20))
        return

    # Simpan session
    user_sessions.setdefault(user_id, {})["bekasankick"] = {"messages": [], "created_at": time.time()}
    ...
    user_sessions[user_id]["bekasankick"]["messages"].append(prompt)
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            if not nomor_list:
                ...
                user_sessions[user_id]["bekasankick"]["messages"].append(gagal_msg)
                asyncio.create_task(expire_session(user_id, "bekasankick", 30))
                return
            ...
            asyncio.create_task(auto_delete_multi(user_id, 30, error))
            user_sessions[user_id]["bekasankick"]["messages"].append(error)
            asyncio.create_task(expire_session(user_id, "bekasankick", 30))
            return
    ...
    user_sessions[user_id]["bekasankick"]["messages"].append(fetching_msg)

    asyncio.create_task(auto_delete_multi(user_id, 5, fetching_msg, nomor_event.message, prompt)) 
    asyncio.create_task(expire_session(user_id, "bekasankick", 5))
    ...
    for nomor_hp in nomor_list:
        ...
        try:
            ...
            slots_terkena_kick = info_panel.get("slots_terkena_kick", [])
            ...
            for slot in data_slot:
                kick_otomatis = slot.get("kick-otomatis", False)
                hapus_status = slot.get("hapus_status", "-")
                slot_lines.append(
                    ...
                    f"    ├ Kick    : {kick_otomatis}\n"
                    f"    └ Status  : {hapus_status}```"
                )
            ...
            msg_per_nomor = (
                ...
                f"└ 🔢 Slot terkena kick: {slots_terkena_kick}\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                f"├ 🎖️ Role: {role}\n"
                f"├ 💵 Harga: Rp {harga:,}\n"
