...
@bot.on(events.CallbackQuery(pattern=b'akrabkick'))
async def akrabkick_slot(event):
    ...
    if saldo < 10000:
        ...
        user_sessions.setdefault(user_id, {}).setdefault("akrabkick", {"messages": []})
        user_sessions[user_id]["akrabkick"]["messages"].append(msg)
        asyncio.create_task(expire_session(user_id, "akrabkick", 20))
        return

    # Simpan session awal
    user_sessions.setdefault(user_id, {})["akrabkick"] = {"messages": [], "created_at": time.time()}

    prompt = await event.respond("📲 Silakan kirim nomor pengelola :", buttons=[
            [Button.inline("❌ Cancel", b"menu")]
        ]
    )
    user_sessions[user_id]["akrabkick"]["messages"].append(prompt)
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            user_sessions[user_id]["akrabkick"]["messages"].append(error)
            asyncio.create_task(expire_session(user_id, "akrabkick", 30))
            return
        ...
        if not nomor_hp.isdigit():
            ...
            user_sessions[user_id]["akrabkick"]["messages"].append(gagal_msg)
            asyncio.create_task(expire_session(user_id, "akrabkick", 30))
            return

        fetching_msg = await event.respond("🔄 Proses mengambil data slot …")
        user_messages[user_id] = fetching_msg
        user_sessions[user_id]["akrabkick"]["messages"].append(fetching_msg)
        ...
        user_sessions[user_id]["akrabkick"]["messages"].append(choose)
        ...
        user_sessions[user_id]["akrabkick"]["messages"].append(proses_msg)
        ...
        final_edit = (
            ...
            "        Detail Sukses\n"
            "╰━━━━━━━━━━━━━━━━━━━━╯\n"
            f"├ 📞 No Admn : {nomor_hp or '-'}\n"
            f"├ 👤 No Anggta : {nomor_anggota or '-'}\n"
            f"├ 🏷️ Nma Anggta : {alias or '-'}\n"
        ...
        # TTL session 2 menit
        asyncio.create_task(auto_delete_multi(user_id, 5, choose, fetching_msg, nomor_event.message, prompt, proses_msg))
        asyncio.create_task(expire_session(user_id, "akrabkick", 5))
