...
async def akrabadd_slot(event):
    ...
    if saldo < 10000:
        ...
        user_sessions.setdefault(user_id, {}).setdefault("akrabadd", {"messages": []})
        user_sessions[user_id]["akrabadd"]["messages"].append(msg)
        asyncio.create_task(expire_session(user_id, "akrabadd", 20))
        return

    # Simpan session awal
    user_sessions.setdefault(user_id, {})["akrabadd"] = {"messages": [], "created_at": time.time()}

    prompt = await event.respond("📲 Silakan kirim nomor pengelola :", buttons=[
            [Button.inline("❌ Cancel", b"menu")]
        ]
    )
    user_sessions[user_id]["akrabadd"]["messages"].append(prompt)
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            user_sessions[user_id]["akrabadd"]["messages"].append(error)
            asyncio.create_task(expire_session(user_id, "akrabadd", 30))
            return
        ...
        if not nomor_hp.isdigit():
            ...
            user_sessions[user_id]["akrabadd"]["messages"].append(gagal_msg)
            asyncio.create_task(expire_session(user_id, "akrabadd", 30))
            return

        fetching_msg = await event.respond("🔄 Proses mengambil data slot …")
        user_messages[user_id] = fetching_msg
        user_sessions[user_id]["akrabadd"]["messages"].append(fetching_msg)
        ...
        user_sessions[user_id]["akrabadd"]["messages"].append(choose)
        ...
        user_sessions[user_id]["akrabadd"]["messages"].append(prompt2)
        ...
        user_sessions[user_id]["akrabadd"]["messages"].append(prompt3)
        ...
        user_sessions[user_id]["akrabadd"]["messages"].append(prompt4)
        ...
        user_sessions[user_id]["akrabadd"]["messages"].append(proses_msg)
        ...
        asyncio.create_task(expire_session(user_id, "akrabadd", 5))
