...
@bot.on(events.NewMessage(pattern=r"(?:.invitcircle|/invitcircle)$"))
@bot.on(events.CallbackQuery(pattern=b'invitcircle'))
async def invitcircle_slot(event):
    ...
    if saldo < 10000:
        ...
        sid = f"invitcircle:{secrets.token_hex(2)}"
    ...
    async with bot.conversation(chat) as conv:
        ...
        try:
            create_resp = await ngundang_api(CIRCLE, invite_payload)
        except Exception as e:
            err = await event.respond(f"❌ Gagal kirim permintaan create: {e}")
            user_messages[user_id] = err
            asyncio.create_task(auto_delete_multi(user_id, 30, err))
            user_sessions[user_id][sid]["messages"].append(err)
            asyncio.create_task(expire_session(user_id, sid, 30))
        ...
        # ===== Normalisasi & Validasi Pola Sukses (sesuai contoh respons barumu) =====
        ...
        srv_code   = (srv.get("code") or "").strip()
        srv_status = (srv.get("status") or "").upper()
        srv_data   = srv.get("data") or {}
        srv_rc     = (srv_data.get("response_code") or "").strip()
        ...
        # ===== Parsing detail sukses dari struktur BARU =====
        details        = data_root.get("details", {}) or {}
        info_panel     = data_root.get("info_saldo_panel", {}) or {}

        nomor_pengelola = details.get("nomor_pengelola") or invite_payload.get("nomor_admin", "-")
        nomor_member    = details.get("nomor_member") or invite_payload.get("nomor_anggota", "-")
        member_name_out = details.get("member_name") or nama_anggota or "-"
        ...
        final_create = (
            ...
            "       Detail Sukses\n"
            "╰━━━━━━━━━━━━━━━━━━━━╯\n"
            f"├ 🧑‍💼 Pengelola : `{nomor_pengelola}`\n"
            f"├ 👤 Anggota    : `{member_name_out}`\n"
            f"└ 📞 No Anggota : `{nomor_member}`\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"├ 🆔 Group ID   : `{group_id}`\n"
            ...
            f"├ 💵 Jasa : Rp 300\n"
            f"└ 💰 Saldo Sisa : "
            f"{('Rp ' + format(saldo_akhir, ',') if isinstance(saldo_akhir, int) else str(saldo_akhir or '-'))}\n"
            f"   📝 Catatan : {catatan_panel}\n"
            "╰━━━━━━━━━━━━━━━━━━━━╯"
        )
        await event.respond(final_create, parse_mode="markdown")

        # TTL session 2 menit
        asyncio.create_task(auto_delete_multi(user_id, 5, fetching_msg, nomor_event.message, prompt, prompt2, nomor_event1.message, prompt4, nama_anggotaa.message, proses_msg))
