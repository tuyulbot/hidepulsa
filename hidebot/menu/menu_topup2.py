...
@bot.on(events.CallbackQuery(data=b'topupppp2'))
async def topup2_handler(event):
    logger.info(f"Command topup2 dipanggil oleh {event.sender_id}")
    ...
    await handle_topup2(event)  # pakai await biar langsung muncul respon

async def handle_topup2(event):
    ...
    inline = [
        [Button.inline(f"QiosPay ({qios_status_text})", b"topp2"),
        Button.inline(f"Orkut ({orkut_status_text})", b"topuporkut2")],
        [Button.inline(f"Tripay ({tripay_status_text})", b"topup")],
