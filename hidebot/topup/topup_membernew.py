...
@bot.on(events.CallbackQuery(data=b'topupmembernew'))
async def topupmembernew(event):
    ...
    for _ in range(10):  # Coba cek status hingga 10 kali
        try:
            ...
            if not success and "PAID" in message.upper():
                ...
                cursor = db.cursor()
                cursor.execute("UPDATE user SET saldo = saldo + %s WHERE id_telegram = %s", (amount, user_id))
...
async def get_topup_amount(sender):
    async with bot.conversation(sender.id) as conv:
        ...
        try:
            ...
            if amount.isdigit() and int(amount) >= 300000:
                return int(amount)
            else:
                await conv.send_message("**Jumlah top-up harus berupa angka dan minimal 300.000.**")
                return None
...
async def send_admin_notification(user_id, user_name, amount):
    admin_id = -1002345639504  # Gantilah dengan ID Telegram admin
    #admin_id = 1316596937
    await bot.send_message(
        admin_id,
        f"━━━━━━━━━━━━━━━━━━━━━━━━━\n"
