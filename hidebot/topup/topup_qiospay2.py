...
@bot.on(events.NewMessage(pattern='/topp2'))
@bot.on(events.CallbackQuery(data=b'topp2'))
async def topp2_handler(event):
    chat = event.chat_id
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            if nominal_int < 10000:
                await event.respond("❌ Minimal topup Rp 10.000")
                return
    ...
    # Cek selama 10 menit (20 x 15 detik)
    for i in range(20):
        ...
        try:
            ...
            if data.get("status") == "success":
                for trx in data.get("data", []):
                    # Filter Validasi (Kredit Only)
                    if trx.get("type") != "CR": continue 
                    ...
                    if trx_amount == nominal_unik:
                        # === LOGIKA TAMBAH SALDO (MySQL) ===
                        
                        user_id = sender.id
                        credit_amount = nominal_unik
                        # Ambil ID transaksi dari QiosPay sebagai referensi unik
                        reference = str(trx.get("id", f"TRX-{nominal_unik}-{int(time.time())}"))

                        try:
                            # 1. Buka Koneksi MySQL
                            db = get_db_connection2()
                            cursor = db.cursor()

                            # 2. Pastikan tabel pencatat history ada
                            _ensure_processed_table(cursor, db)

                            # 3. Cek apakah referensi ini sudah pernah diproses?
                            if _has_been_processed(cursor, db, reference):
                                print(f"[Topup] Reference {reference} sudah diproses, skip.")
                                sukses = True 
                                break

                            # 4. Eksekusi Update Saldo (Syntax MySQL pakai %s)
                            cursor.execute(
                                "UPDATE ress SET saldo = saldo + %s WHERE id_telegram = %s",
                                (credit_amount, str(user_id))
                            )
                            
                            # 5. Catat Reference agar tidak double claim
                            cursor.execute(
                            ...
                            logger.error(f"Error Database Update: {e}")
                            if db: db.rollback() # Rollback jika error di tengah jalan
                        finally:
                            # Tutup koneksi
                            try:
                                if db: db.close()
                        ...
                        break # Break dari loop transaksi (for trx)
            
            if sukses: break # Break dari loop polling (for i)
