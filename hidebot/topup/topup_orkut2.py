...
@bot.on(events.NewMessage(pattern='/topuporkut2'))
@bot.on(events.CallbackQuery(data=b'topuporkut2'))
async def toporkut2_handler(event):
    chat = event.chat_id
                ...
                for trx in results:
                    ...
                    if trx_amount == nominal_unik:
                        ...
                        try:
                            ...
                            cursor.execute("UPDATE ress SET saldo = saldo + %s WHERE id_telegram = %s", (nominal_unik, str(user_id)))
