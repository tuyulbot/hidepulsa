...
@bot.on(events.CallbackQuery(pattern=b'tri_login'))
async def tri_login(event):
    user_id = event.sender_id
    chat = event.chat_id
    sender = await event.get_sender()
    print("DEBUG: tri_login TRIGGERED", user_id, time.time())
    ...
    # BUAT SESSION
    # ==========================
    user_sessions.setdefault(user_id, {})["tri_login"] = {
        "messages": [],
    ...
    user_sessions[user_id]["tri_login"]["messages"].append(prompt)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                user_sessions[user_id]["tri_login"]["messages"].append(msg)
    ...
    asyncio.create_task(expire_session(user_id, "tri_login", 60))
...
async def send_err(event, user_id, text):
    ...
    user_sessions[user_id]["tri_login"]["messages"].append(err)
    asyncio.create_task(expire_session(user_id, "tri_login", 30))
    return